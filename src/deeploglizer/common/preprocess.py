...
class FeatureExtractor(BaseEstimator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert embedding_type in ['tfidf', 'w2v']
    ...
    def fit(self, session_dict):
        ...
        self.id2log_train.update(
            {idx: log for idx, log in enumerate(self.ulog_train, 2) if log not in self.id2log_train.values()}  # remove those potential duplicate log messages.
        )
        ...
        elif self.label_type in ["anomaly", "none"]:
    ...
    def transform(self, session_dict, datatype="train"):
        ...
        if self.feature_type == "semantics":
            ...
            log2idx = {log: indice[idx].astype('float32') for idx, log in enumerate(ulog)}
            ...
            log2idx["PADDING"] = np.zeros(len(list(log2idx.values())[0])).reshape(-1).astype('float32')
